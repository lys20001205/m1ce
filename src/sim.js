...
export class Game {
 ...
 attack(){if(this.status!=='running'||this.paused)return false;const p=this.player;if(p.cooldown>0||p.stun>0||p.carry)return false;p.cooldown=this.round>=3?.43:.50;p.swing=.36;p.swingHit=false;
  if(this.round>=3){const m=this.muzzle();this.lastMuzzle={...m};this.projectiles.push({id:this.nextId++,x:m.x,y:m.y,origin:m.x,dir:p.face,roof:p.roof,life:.7});this.feedback('muzzle',m.x,m.y);this.tell('projectile_spawn',{...m,roof:p.roof})}this.tell('attack',{x:p.x,roof:p.roof,weapon:this.weapon});return true}
 muzzle(){return{x:this.player.x+this.player.face*1.08605,y:this.player.y+1.1235}}
