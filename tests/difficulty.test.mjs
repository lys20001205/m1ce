...
test('staying critical never becomes permanent spawn immunity',()=>{
 const g=new Game({seed:314159});g.chooseRoute('industrial');g.chooseCar('cargo');g.start();g.round=5;g.t=.121;g.phase='yard';g.cars[0].hp=20;g.player.roof=true;g.player.y=4.12;
...
test('healing and redropping health cannot farm repeated distress windows',()=>{
 const g=new Game();g.chooseRoute('industrial');g.chooseCar('cargo');g.start();g.round=4;g.t=.12;g.cars[0].hp=20;tick(g,.025);
