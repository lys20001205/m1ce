...
## Native partial-lift driver correction
Run 36242872746 passed all 49 WebKit common checks, four layout sizes and native mouse/tap flows. Chromium confirmed two-finger move/attack and sliding, then the driver requested touchEnd for the ATTACK point while expecting movement to end. An isolated blank-DOM Chromium protocol probe confirmed which native pointerup was dispatched. The driver now targets the movement point and asserts the actual pointerup/cancel target and surviving input, retaining the failed artifact. No game-input workaround was added. Ranged captions now explain the next unlock/purchase step; the help text describes sliding and the neutral center. Full amended CI still required.
