"""G regression fixes: synchronous UI refresh, robust readiness and stop anti-toggle quota."""
from pathlib import Path
import hashlib
checks={'src/sim.js': ['ea6208acddf2158631e7c16895e897c887d3b52daa9804dbb1cc2f7a2db9a6f3', 'd3fff2a895694016ea50f0e34699adfb5e46feaa95d7647f45bb4bc3b9b3e1f0'], 'src/main.js': ['367e8c95193db1decc9b978d2f64b844265f542eddc251fd2983e5b68f29e65c', '12dac199921a731bcca64c81b9d732c90a98538311fb0998a01d43a81fbca787'], 'tests/combat.test.mjs': ['bbf6f1b4309c62512a6fee7a697c51f9cb8d1907dda343e98df72d5b36944b94', 'bb3f43e1bb4c95a6d4889c984f4f554eb53b3259f0d4539394ccd8ab91fd85e4'], 'tests/browser_v11_combat.py': ['d9fd8521644570382fe35e1fe98d91fc21f28c3253eae9be8277b53ee4b86d94', 'ddfb25a573129dc87062fee4fe3abde3d4c60b27f12464dd6db87388349f8556']}
...
p.write_text(s)
p=Path('src/main.js');s=p.read_text().replace("$('closeArmory').onclick=()=>{game.armoryOpen=false;};", "$('closeArmory').onclick=()=>game.closeArmory();").replace(".onclick=()=>game.buyWeapon(slot);", ".onclick=()=>{game.buyWeapon(slot);ui();};");p.write_text(s)
...
p=Path('tests/browser_v11_combat.py');p.write_text(p.read_text().replace("'__RH_DEBUG?.snapshot()", "'window.__RH_DEBUG?.snapshot()"))
